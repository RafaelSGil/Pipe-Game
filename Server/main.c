...
// Variables
	...
	if (argc != 4) {
		if (RegCreateKeyEx(HKEY_CURRENT_USER, registry.keyCompletePath, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &registry.key, &registry.dposition) == ERROR_SUCCESS)
		{
			...
			long rowsRead = RegQueryValueEx(registry.key, TEXT("Rows"), NULL, REG_DWORD, (LPBYTE)&game.rows, sizeof(DWORD));
			long columnsRead = RegQueryValueEx(registry.key, TEXT("Columns"), NULL, REG_DWORD, (LPBYTE)&game.columns, sizeof(DWORD));
			long timeRead = RegQueryValueEx(registry.key, TEXT("Time"), NULL, REG_DWORD, (LPBYTE)&game.time, sizeof(DWORD));
			...
			_tprintf_s(TEXT("\nCouldnt create the desired key!"));
			return -1;
		}
	}else {
		game.rows = (DWORD) argv[1];
		game.columns = (DWORD) argv[2];
		game.time = (DWORD) argv[3];
		_tprintf(TEXT("%d\n"), game.rows);
		_tprintf(TEXT("%d\n"), game.columns);
	}
